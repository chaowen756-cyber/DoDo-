...
class DOE(Layer):
    ...
    def __init__(self, Mdoe=128, Mesce=128, wave_lengths=None, DOE_type='New', Trai=True, **kwargs):
        ...
        if DOE_type == 'New' or DOE_type == 'Zeros':
            self.Trai = Trai #Trai
            self.zernike_volume = loadmat('/content/drive/MyDrive/HDSP/Double doe/Multishot_128x128/Base_zernike_128x128_nopadd.mat').get('HmBase').astype(np.float32)
        else:
            Hm_DOE = loadmat('/content/drive/MyDrive/HDSP/Double doe/Multishot_128x128/Spiral_128x128_nopadd.mat').get('Hm').astype(np.float32)
            self.Hm_DOE = Hm_DOE
            
            self.Trai = False
        self.P = loadmat('/content/drive/MyDrive/HDSP/Double doe/Multishot_128x128/Spiral_128x128_nopadd.mat').get('P').astype(np.float32)
    ...
    def build(self, input_shape):
        if self.DOE_type == 'New':
            ...
            self.zernike_coeffs = self.add_weight(name='zernike_coeffs', shape=zernike_inits.shape,
                                                  constraint=MinMaxNorm(min_value=-1.0, max_value=1.0, rate=1.0, axis=2),
