...
class Propagation(Layer):
    ...
    def build(self, input_shape):
        ...
        self.z = self.add_weight(name="Distance", shape=[1], constraint=NonNeg(), initializer=initializerC, trainable=self.Trai)
    ...
    def call(self, input, **kwargs):
        ...
        #Mdo = K.floor(Mp * r)
        #if r < 1:
         #   u2 = u2[int(Mp / 2 - Mdo / 2): int(Mp / 2 + Mdo / 2),
          #       int(Mp / 2 - Mdo / 2): int(Mp / 2 + Mdo / 2)]
...
class PropagationABS(Layer):
    ...
    def build(self, input_shape):
        initializerC = K.constant_initializer(self.zi)
        self.z = self.add_weight(name="Distance", shape=[1], initializer=initializerC, constraint=NonNeg(), trainable=self.Trai)
...
class Layer_H(Layer):
    ...
    def build(self, input_shape):
        self.kernel = self.add_weight(name="kernel", shape=input_shape[1:4], initializer="random_normal", trainable=True)
