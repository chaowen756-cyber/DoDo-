...
case "${1:-}" in
  build-index)
    ...
    ;;
  stage-a-balanced)
    run_training psfconv_number_18_eb4a_balanced_only_stageA_12ep "${MAX_EPOCHS:-12}" balanced stage_a
    ;;
  stage-a-augment)
    run_training psfconv_number_18_eb4b_baek_augment_only_stageA_12ep "${MAX_EPOCHS:-12}" augment stage_a
    ;;
  stage-a-combined)
    run_training "${STAGE_A_COMBINED_NAME:-psfconv_number_18_eb4c_baek_balanced_stageA_12ep}" "${MAX_EPOCHS:-12}" combined stage_a
    ;;
  stage-b)
    if [[ -z "${INIT_CKPT:-}" ]]; then
    ...
    run_training "${STAGE_B_NAME:-psfconv_number_18_eb4d_${VARIANT}_stageB_30ep}" "${MAX_EPOCHS:-30}" "${VARIANT}" stage_b "${INIT_CKPT}"
    ;;
  *)
